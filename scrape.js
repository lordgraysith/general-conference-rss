...
var attr = (el, name) => {
	...
	return attributes.where(attr => attr.name === name).select(attr => attr.value)[0]
}
...
var generateData = function(dateIn, month, year){
	...
	elementArray.forEach(function(el){
		try {
			...
			title = el.getElementsByClassName('lumen-tile__title')[0].textContent.trim();
			author = el.getElementsByClassName('lumen-tile__content')[0].textContent.trim();
			...
			date = new Date(today);
			promises.push(get(link).then(function(pageData){
				var page = parseHtml(pageData);
				description = page.getElementsByClassName('article-author__name')[0].textContent.trim()
					+ ' ' + page.getElementsByClassName('article-author__title')[0].textContent.trim();
				mp3 = Array.prototype.slice.apply(
